...
def onNewDeviceConnected(deviceName):
    ...
    if handler:
        ...
        # below is an example of using channel 1
        experimentChannel = 0

        # start Manual Experiment on the channel specified above.
        ...
        # set Sampling intervierval (1 s) for Manual Experiment
        error = handler.setManualModeSamplingInterval(experimentChannel, 1)
        if error.value() != AisErrorCode.ErrorCode.Success:
            handler.stopExperiment(experimentChannel)
            print(error.message())
            return
